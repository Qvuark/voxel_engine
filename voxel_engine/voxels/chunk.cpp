...
Chunk::Chunk(int xpos, int ypos, int zpos) : x(xpos), y(ypos), z(zpos)
{
    ...
    const float SURFACE_ENTRY_CHANCE = 0.02f;
    const int ENTRY_RADIUS = 6;
    const int SHAFT_RADIUS = 3;
    const int MIN_ENTRY_DEPTH = 5;
    const int WATER_CHECK_RADIUS = 10;

    voxels = new Voxel[CHUNK_VOLUME]{};

    
    int surfaceHeightCache[CHUNK_WIDTH][CHUNK_DEPTH];
    for (int lz = 0; lz < CHUNK_DEPTH; lz++)
    {
        for (int lx = 0; lx < CHUNK_WIDTH; lx++)
        {
            ...
            surfaceHeightCache[lx][lz] = (WATER_LEVEL - 20) + static_cast<int>((noise + 1.0f) * 25.0f);
        }
    }
    ...
    for (int ly = 0; ly < CHUNK_HEIGHT; ly++)
    {
        ...
        bool isCaveLayer = (realY >= CAVE_MIN_Y && realY - 5 <= CAVE_MAX_Y);
        float mask = 1.0f;

        if (isCaveLayer)
        {
            float t = static_cast<float>(realY - CAVE_MIN_Y) / (CAVE_MAX_Y - CAVE_MIN_Y);
            mask = 1.0f - 4.0f * (t - 0.5f) * (t - 0.5f);
        }
        ...
        for (int lz = 0; lz < CHUNK_DEPTH; lz++)
        {
            for (int lx = 0; lx < CHUNK_WIDTH; lx++)
            {
                ...
                int realX = lx + x * CHUNK_WIDTH;
                int realZ = lz + z * CHUNK_DEPTH;
                int terrainHeight = surfaceHeightCache[lx][lz];
                ...
                if (voxels[idx].id == 0)
                {
                    ...
                    if (realY <= terrainHeight && realY < CHUNK_HEIGHT)
                    {
                        ...
                        {
                            id = 1;
                            if (rand() % COAL_SPAWN_CHANCE == 0) id = 7; 
                        }
                ...
                {
                    glm::vec3 warpPos(
                        realX * CAVE_NOISE_SCALE * 0.3f,
                        realZ * CAVE_NOISE_SCALE * 0.3f,
                        realY * VERTICAL_SQUASH * 0.1f
                    );

                    glm::vec3 q(
                        glm::perlin(warpPos + glm::vec3(0.0f, 5.2f, 1.7f)),
                        glm::perlin(warpPos + glm::vec3(3.5f, 2.8f, 8.1f)),
                        glm::perlin(warpPos + glm::vec3(9.7f, 4.3f, 6.6f))
                    );

                    glm::vec3 warped = warpPos + q * CAVE_WARP_INTENSITY;

                    float n1 = glm::perlin(glm::vec3(
                        realX * CAVE_NOISE_SCALE * HORIZONTAL_STRETCH,
                        realZ * CAVE_NOISE_SCALE * HORIZONTAL_STRETCH,
                        realY * VERTICAL_SQUASH
                    ));

                    float n2 = glm::perlin(warped * 3.0f) * 0.4f;
                    float n3 = glm::perlin(glm::vec3(
                        realX * 0.15f,
                        realZ * 0.15f,
                        realY * 0.05f
                    )) * 0.2f;
                    ...
                    if (noise > CAVE_THRESHOLD)
                    {
                        ...
                        int surfaceHeight = surfaceHeightCache[lx][lz];
                        ...
                        {
                            for (int dx = -WATER_CHECK_RADIUS; dx <= WATER_CHECK_RADIUS && !nearWater; dx++)
                            {
                                for (int dz = -WATER_CHECK_RADIUS; dz <= WATER_CHECK_RADIUS && !nearWater; dz++)
                                {
                                    if (dx == 0 && dz == 0) continue;

                                    int checkX = (realX + dx) / CHUNK_WIDTH;
                                    int checkZ = (realZ + dz) / CHUNK_DEPTH;

                                   
                                    if (checkX == x && checkZ == z)
                                    {
                                        int localX = (realX + dx) % CHUNK_WIDTH;
                                        int localZ = (realZ + dz) % CHUNK_DEPTH;
                                        if (surfaceHeightCache[localX][localZ] <= WATER_LEVEL)
                                        {
                                            nearWater = true;
                                            break;
                                        }
                        ...
                        if (!nearWater &&
                            depth >= MIN_ENTRY_DEPTH &&
                            realY >= CAVE_MAX_Y - 15 &&
                            mask > 0.7f &&
                            noise > CAVE_THRESHOLD * 1.5f &&
                            (rand() % 1000) < (SURFACE_ENTRY_CHANCE * 1000))
                        {
                            glm::vec3 entryWarp(
                                realX * 0.1f,
                                realZ * 0.1f,
                            ...
                            for (int dy = 0; dy <= depth; dy++)
                            {
                                ...
                                if (currentY > CAVE_MAX_Y) break;

                                float t = static_cast<float>(dy) / depth;
                                int offsetX = lx + static_cast<int>(warpX * t * 4.0f);
                                int offsetZ = lz + static_cast<int>(warpZ * t * 4.0f);

                                for (int ox = -SHAFT_RADIUS; ox <= SHAFT_RADIUS; ox++)
                                {
                                    for (int oz = -SHAFT_RADIUS; oz <= SHAFT_RADIUS; oz++)
                                    {
                                        if (ox * ox + oz * oz > SHAFT_RADIUS * SHAFT_RADIUS) continue;
                                        ...
                                        if (voxels[entryIdx].id == 5 ||
                                            voxels[entryIdx].id == 6 ||
                                            voxels[entryIdx].id == 9 ||
                                            voxels[entryIdx].id == 10) continue;

                                        if (dy > depth - 3)
                                        {
                                            float radiusMod = (ENTRY_RADIUS * ENTRY_RADIUS) * (1.0f - t);
                                            if (ox * ox + oz * oz <= radiusMod)
                                            {
                    ...
                    (rand() % 200) == 0)
                {
                    float dx = lx - 8;
                    float dz = lz - 8;
                    float dy = ly - (CAVE_MAX_Y - CAVE_MIN_Y) / 2;
